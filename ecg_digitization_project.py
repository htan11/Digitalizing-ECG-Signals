...
This script creates a reproducible CNN/U-Net-territory experiment for ECG image
digitization using the sample PhysioNet-style folders already present in this
workspace. It trains a compact image-to-signal CNN on generated ECG paper strips,
compares it with baseline digitizers, evaluates regression metrics, and then
demonstrates a downstream multimodal ResNet-style image branch + LSTM signal
branch classifier for amplitude-bin classification.
...
Outputs:
  outputs/
    ...
    predictions/*_prediction.csv
    plots/*.png
    results_report.md

The generated ECG strip images are used as supervised "physical paper" inputs
because the provided sample data has CSV time series but no per-pixel annotation
for every waveform segment on the scanned page. The original ECG page images are
still used in the physical-vs-digital visualization.
"""
...
- Data used: `{n_records}` record folders and `{n_samples}` lead-level samples from `dl-lms-data/sample_data`.
- Digitization model: compact CNN/U-Net-style image encoder trained to regress a normalized `{hp.signal_length}` point waveform from an ECG-paper strip image.
...
|---|---:|
| Seed | {hp.seed} |
| Signal length | {hp.signal_length} |
| Image size | {hp.image_height} x {hp.image_width} |
| Digitizer epochs | {hp.digitizer_epochs} |
| Classifier epochs | {hp.classifier_epochs} |
| Batch size | {hp.batch_size} |
| Learning rate | {hp.learning_rate} |
| Validation size | {hp.validation_size} |
| Test size | {hp.test_size} |
| CNN filters | {hp.conv_filters} |
| Dropout | {hp.dropout} |
| LSTM units | {hp.lstm_units} |

## Regression Results

{dataframe_to_markdown(regression_df)}

Best digitization result by RMSE: **{best_reg['model']}** with RMSE `{best_reg['RMSE']:.4f}`, MAE `{best_reg['MAE']:.4f}`, MAPE `{best_reg['MAPE_%']:.2f}%`, and R2 `{best_reg['R2']:.4f}`.
...
{dataframe_to_markdown(classification_df)}

Best downstream classifier by macro F1: **{best_cls['model']}** with accuracy `{best_cls['Accuracy']:.4f}` and macro F1 `{best_cls['F1_macro']:.4f}`.
